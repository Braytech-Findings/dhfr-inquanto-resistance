...
def args(tmp_path: Path, **updates) -> Namespace:
    values = dict(
        backend="H2-Emulator", shots=10, max_hqc=0.0,
        confirm_hardware=False, confirm_submit=False, user_group="research-group",
        require_user_group=False, dry_run=True, compile_only=False,
        project_id="project-id", project_name=None, timeout=1, wait=False,
...
def test_authentication_does_not_mark_entitlement(tmp_path: Path, monkeypatch) -> None:
    class Devices:
        ...
        def get_all():
            import pandas as pd
            return SimpleNamespace(df=lambda: pd.DataFrame({"device_name": ["H2-Emulator"]}))
...
def test_access_code_14_records_resolution_without_retry(tmp_path: Path, monkeypatch) -> None:
    class FakeQnx:
        projects = SimpleNamespace(get=lambda **_: SimpleNamespace(id="project-id"))
        circuits = SimpleNamespace(upload=lambda **_: (_ for _ in ()).throw(RuntimeError("access code 14")))
        login = staticmethod(lambda: None)
    monkeypatch.setattr(nexus, "load_nexus", lambda: (FakeQnx(), object))
    monkeypatch.setattr(nexus, "bell", lambda: SimpleNamespace(n_qubits=2, n_gates=1, depth=lambda: 1))
...
def test_retrieval_never_creates_replacement_job(tmp_path: Path, monkeypatch) -> None:
    ...
    fake = SimpleNamespace(
        ...
        start_execute_job=lambda **_: calls.__setitem__("execute", calls["execute"] + 1),
    )
    monkeypatch.setattr(nexus, "load_nexus", lambda: (fake, object))
    ...
    assert json.loads(call_args.metadata_output.read_text())["replacement_job_created"] is False
