...
def test_4dtmp_generation_preserves_retained_coordinates(tmp_path):
    ...
    mol = next(mol for mol in Chem.SDMolSupplier(str(output), removeHs=False) if mol is not None)
    heavy = Chem.RemoveHs(mol)
    assert heavy.GetNumAtoms() == 20

    source = module.load_ligand_from_pdb(ROOT / "data/raw/pdbs/6XG5.pdb", residue_name="TOP")
