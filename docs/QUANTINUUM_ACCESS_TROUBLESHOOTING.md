...
`scripts/submit_to_hardware_direct.py` is now explicitly a **state-circuit smoke test**. A parameterized UCCSD state-preparation circuit does not calculate molecular energy; the complete result requires the exact 576 grouped Hamiltonian measurement circuits and Pauli reconstruction. The guarded hosted-energy prerequisite checker intentionally refuses submission until that mapping has been independently validated.
