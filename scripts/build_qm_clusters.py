    ...
    for system in SYSTEMS:
        ...
        for residue in pdb.topology.residues():
            if residue.name not in PROTEIN:
                continue
            heavy = [atom.index for atom in residue.atoms() if atom.element.symbol != "H"]
            distance = np.linalg.norm(positions[heavy, None, :] - positions[ligand][None, :, :], axis=2).min()
            if distance <= cutoff:
                selected.add(int(residue.id))
    return sorted(selected)


def link_position(anchor: np.ndarray, excluded: np.ndarray, length: float) -> np.ndarray:
...
def build(
    ...
    positions = xyz(pdb)
    protein_residues = [residue for residue in pdb.topology.residues() if residue.name in PROTEIN]
    by_number = {int(residue.id): residue for residue in protein_residues}
    selected = set(residues)
    ligand_residue = next(residue for residue in pdb.topology.residues() if residue.name in {"TOP", "DTM"})
    ligand_heavy = [atom.index for atom in ligand_residue.atoms() if atom.element.symbol != "H"]
    atoms: list[dict[str, object]] = []

    def append_atom(element: str, position: np.ndarray, fragment: str, label: str) -> None:
        atoms.append({"element": element, "xyz": position, "fragment": fragment, "label": label})

    for number in residues:
        residue = by_number[number]
        for atom in residue.atoms():
            append_atom(atom.element.symbol, positions[atom.index], "environment", f"{residue.name}{number}:{atom.name}")
    for atom in ligand_residue.atoms():
        append_atom(atom.element.symbol, positions[atom.index], "ligand", f"{ligand_residue.name}:{atom.name}")
    ...
    if water_cutoff is not None:
        for residue in pdb.topology.residues():
            ...
            distance = np.linalg.norm(positions[oxygen.index] - positions[ligand_heavy], axis=1).min()
            if distance <= water_cutoff:
                retained_waters += 1
                for atom in residue.atoms():
                    append_atom(atom.element.symbol, positions[atom.index], "environment", f"water{residue.id}:{atom.name}")
    ...
    for index, residue in enumerate(protein_residues):
        ...
        if index > 0 and int(protein_residues[index - 1].id) not in selected:
            ...
            if "N" in atom_by_name and "C" in previous:
                append_atom(
                    "H", link_position(positions[atom_by_name["N"].index], positions[previous["C"].index], 1.01),
                    "environment", f"link:N{number}",
                )
                link_count += 1
        if index + 1 < len(protein_residues) and int(protein_residues[index + 1].id) not in selected:
            following = {atom.name: atom for atom in protein_residues[index + 1].atoms()}
            if "C" in atom_by_name and "N" in following:
                append_atom(
                    "H", link_position(positions[atom_by_name["C"].index], positions[following["N"].index], 1.09),
                    "environment", f"link:C{number}",
                )
                link_count += 1
    ...
    pd.DataFrame(
        [{"atom_index": i, "element": atom["element"], "fragment": atom["fragment"], "label": atom["label"]}
...
def main() -> None:
    ...
    for tier, definition in config["tiers"].items():
        ...
        if residues is None:
            residues = selected_union(float(definition["protein_cutoff_angstrom"]), suffix)
        for system in SYSTEMS:
            rows.append(build(
                system, tier, residues, definition["water_cutoff_angstrom"], suffix,
