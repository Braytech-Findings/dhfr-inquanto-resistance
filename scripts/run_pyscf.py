...
def main() -> None:
    ...
    parser.add_argument(
        "--spin", type=int, default=0, help="2S = unpaired alpha minus beta electrons"
    )
    parser.add_argument("--memory", type=int, default=4000)
    parser.add_argument("--save-orbitals", type=Path)
    parser.add_argument(
        "--checkpoint", type=Path, help="PySCF checkpoint for AVAS/restart"
    )
    ...
    mf = (
        scf.RHF(mol)
        if args.method == "HF" and args.spin == 0
        else scf.UHF(mol)
        if args.method == "HF"
        else dft.RKS(mol)
        if args.spin == 0
