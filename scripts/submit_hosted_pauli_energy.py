...
def main() -> None:
    ...
    checkpoint = ROOT / "results/quantum/protocol_checkpoints" / f"{args.system}_H2-1LE_{args.shots_per_circuit}shots_built.pkl"
    plan = ROOT / "results/quantum/measurement_plans" / f"{args.system}_H2-1LE_{args.shots_per_circuit}shots_plan.json"
    missing = [str(path.relative_to(ROOT)) for path in (checkpoint, plan) if not path.exists()]
    if missing:
        raise SystemExit("Required ignored local artifacts are missing: " + ", ".join(missing))
    print("Prerequisites found, but no hosted molecular-energy submission path is enabled.")
    print("Reason: the repository has not yet validated an exact, checkpoint-preserving mapping of all PauliAveraging circuits to Nexus batches.")
