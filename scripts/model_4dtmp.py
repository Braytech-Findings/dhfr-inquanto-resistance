    ...
    for oxygen in mol.GetAtoms():
        ...
        if aromatic and methyl:
            ...
            candidates.append(
                (
                    len(
                        Chem.GetShortestPath(mol, anchor.GetIdx(), ring_carbon.GetIdx())
                    ),
                    oxygen,
                    methyl[0],
                )
...
def transform(mol: Chem.Mol) -> Chem.Mol:
    ...
    attached_hydrogens = [
        atom.GetIdx()
        for atom in mol.GetAtomWithIdx(methyl_idx).GetNeighbors()
...
def main() -> None:
    ...
    parser.add_argument(
        "input", type=Path, help="TMP ligand as an SDF/MOL file with 3D coordinates"
    )
    parser.add_argument("output", type=Path)
    args = parser.parse_args()
    supplier = (
        Chem.SDMolSupplier(str(args.input), removeHs=False)
        if args.input.suffix.lower() == ".sdf"
        else [Chem.MolFromMolFile(str(args.input), removeHs=False)]
    )
