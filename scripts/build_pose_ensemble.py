...
def linker_dihedral(molecule: Chem.Mol) -> tuple[int, int, int, int]:
    ...
    for atom in molecule.GetAtoms():
        ...
        aromatic = [
            neighbor for neighbor in heavy_neighbors if neighbor.GetIsAromatic()
        ]
        if len(aromatic) != 2:
            continue
        phenyl = next(
            (
                candidate
                for candidate in aromatic
                if all(n.GetAtomicNum() == 6 for n in candidate.GetNeighbors())
            ),
            None,
        )
        if phenyl is None:
            continue
        pyrimidine = next(
            candidate for candidate in aromatic if candidate.GetIdx() != phenyl.GetIdx()
        )
        phenyl_neighbor = next(
            neighbor
            for neighbor in phenyl.GetNeighbors()
...
def protein_coordinates(path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    for atom in pdb.GetAtoms():
        ...
        if atom.GetAtomicNum() <= 1 or (
            info and info.GetResidueName().strip() in {"TOP", "DTM"}
        ):
            continue
...
def score(
    molecule: Chem.Mol, protein: np.ndarray, polar: np.ndarray
) -> dict[str, float | int]:
    ...
    polar_distances = np.linalg.norm(
        ligand_polar[:, None, :] - polar[None, :, :], axis=2
    )
    return {
        "min_protein_distance_A": float(distances.min()),
        "clashes_lt_1_8A": int((distances < 1.8).sum()),
        "polar_pairs_2_4_to_3_5A": int(
            ((polar_distances >= 2.4) & (polar_distances <= 3.5)).sum()
        ),
    }
...
def main() -> None:
    ...
    for background in ("WT", "L28R"):
        ...
        protein, polar = protein_coordinates(
            PROCESSED / f"{background}_4DTMP_minimized.pdb"
        )
        ...
        for label, offset in (
            ("inherited", 0.0),
            ("torsion_minus30", -30.0),
            ("torsion_plus30", 30.0),
        ):
            candidate = Chem.Mol(source)
            rdMolTransforms.SetDihedralDeg(
                candidate.GetConformer(), *dihedral, base + offset
            )
        ...
        alternatives = sorted(
            ...
            key=lambda row: (
                row["clashes_lt_1_8A"],
                -row["polar_pairs_2_4_to_3_5A"],
                abs(row["torsion_offset_deg"]),
            ),
        )
