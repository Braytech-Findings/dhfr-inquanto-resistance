...
def ndp_template() -> tuple[dict[str, float], list[tuple[str, str]]]:
    ...
    residue = next(node for node in root.findall("./Residues/Residue") if node.attrib["name"] == "NDP")
    charges = {atom.attrib["name"]: float(atom.attrib["charge"]) for atom in residue.findall("Atom")}
    bonds = [(bond.attrib["atomName1"], bond.attrib["atomName2"]) for bond in residue.findall("Bond")]
...
def main() -> None:
    ...
    for background, pdb_id in (("WT", "6XG5"), ("L28R", "6XG4")):
        ...
        ndp = next(residue for residue in pdb.topology.residues() if residue.name == "NDP")
        ...
        for name, charge in charges.items():
            if name in deposited_names:
                continue
            parent = next(
                (right if left == name else left for left, right in bonds
                 if (left == name and right in deposited_names) or (right == name and left in deposited_names)),
                None,
            )
            if parent is None:
                raise ValueError(f"{background}: cannot collapse missing atom {name} onto a deposited heavy atom")
            collapsed[parent] += charge
        records = []
        for atom in ndp.atoms():
            if atom.name not in collapsed:
                raise ValueError(f"{background}: no CHARMM36 charge for NDP atom {atom.name}")
        ...
        if not np.isclose(total, -3.0, atol=1e-8):
            raise ValueError(f"{background}: expected CHARMM36 NDP charge -3, obtained {total}")
