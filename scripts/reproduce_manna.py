...
def main() -> None:
    if not RAW.exists():
        raise SystemExit(
            "Missing raw source. Clone the repository recorded in configs/source_manifest.yaml"
        )
    ...
    evolution = pd.read_csv(
        RAW / "Figure_4/Figure_4a_4b_4c/DataWithHillFitParameters.csv"
    )
    ...
    final = (
        evolution[evolution["Day#"].eq(21)]
        .groupby(["EvolvedIn", "Culture#"])["fIC50-A"]
        .mean()
    )
    ...
    rows = [
        {
            "metric": "L28R median IC95",
            "tmp_value": l28r_tmp,
            "dtmp_value": l28r_dtmp,
            "ratio_tmp_to_dtmp": l28r_tmp / l28r_dtmp,
            "unit": "ug/mL",
            "source_figure": "2b-2c",
        },
        {
            "metric": "L28R day-21 mutation frequency",
            "tmp_value": float(l28r_frequency.TMP),
            "dtmp_value": float(l28r_frequency.V041),
            "ratio_tmp_to_dtmp": float(l28r_frequency.TMP / l28r_frequency.V041),
            "unit": "percent",
            "source_figure": "4e",
        },
        {
            "metric": "Mean day-21 fitted IC50",
            "tmp_value": float(final_tmp.mean()),
            "dtmp_value": float(final_dtmp.mean()),
            "ratio_tmp_to_dtmp": float(final_tmp.mean() / final_dtmp.mean()),
            "unit": "ug/mL",
            "source_figure": "4a-4c",
        },
    ]
    ...
    axes[1].bar(
        labels, [l28r_frequency.TMP, l28r_frequency.V041], color=["#777777", "#159d9a"]
    )
